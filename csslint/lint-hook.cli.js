...
function cli(api) {
	...
	function gatherRules(options, ruleset) {
		var warnings = options.rules || options.warnings, errors = options.errors;
	...
	function filterRules(options) {
		var ignore = options.ignore, ruleset = null;
	...
	function processFile(relativeFilePath, options) {
		var input = api.readFile(relativeFilePath), ruleset = filterRules(options), result = CSSLint.verify(input, gatherRules(options, ruleset)), formatter = CSSLint.getFormatter(options.format || "text"), messages = result.messages || [], output, exitCode = 0;
	...
	if(files.length === 0) {
		files = files.concat(api.getFiles("."));
	}
